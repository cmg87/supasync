...
function validateComponent(raw: string): string {
  if (raw.length === 0 || raw === "." || raw === "..") {
    throw new ProtocolError("INVALID_PATH", "path component is empty or a traversal segment", {
      component: raw,
    });
  }
  if (raw.includes("\u0000") || /[\u0000-\u001F\u007F]/.test(raw)) {
    throw new ProtocolError("INVALID_PATH", "path component contains a control character", {
      component: raw,
    });
  }
  if (raw.includes("/") || raw.includes("\\") || raw.includes(":")) {
    throw new ProtocolError("INVALID_PATH", "path component contains a reserved separator", {
      component: raw,
    });
  }
  const nfc = raw.normalize("NFC");
  if (nfc.endsWith(" ") || nfc.endsWith(".")) {
    throw new ProtocolError("INVALID_PATH", "path component has a trailing space or dot", {
      component: raw,
    });
  }
  const folded = foldPathComponent(nfc);
  if (isReservedDeviceName(folded)) {
    throw new ProtocolError("INVALID_PATH", "path component is a reserved device name", {
      component: raw,
    });
  }
  const bytes = new TextEncoder().encode(nfc);
  if (bytes.length > DEFAULT_LIMITS.maxComponentBytes) {
    throw new ProtocolError("LIMIT_EXCEEDED", "path component exceeds the maximum length", {
      component: raw,
...
export function canonicalizePath(input: string): CanonicalPath {
  if (typeof input !== "string" || input.length === 0) {
    throw new ProtocolError("INVALID_PATH", "path must be a non-empty relative vault path");
  }
  if (input.includes("\u0000")) {
  ...
  if (replaced !== input) {
    throw new ProtocolError("INVALID_PATH", "backslash separators are not allowed");
  }
  if (replaced.startsWith("./") || replaced === "." || replaced.startsWith("../")) {
    throw new ProtocolError("INVALID_PATH", "paths must be relative without a leading dot segment");
  }
  ...
  if (bytes.length > DEFAULT_LIMITS.maxPathBytes) {
    throw new ProtocolError("LIMIT_EXCEEDED", "path exceeds the maximum length", {
...
export function joinDisplayPath(parent: string | null, name: string): string {
  ...
  if (leaf.includes("/")) {
    throw new ProtocolError("INVALID_PATH", "file name must be a single component");
  }
  if (!parent) return leaf;
  return canonicalizePath(`${parent}/${leaf}`).display;
}

export function replacePrefix(display: string, fromPrefix: string, toPrefix: string): string {
  if (display === fromPrefix) return toPrefix;
  if (!display.startsWith(`${fromPrefix}/`)) {
    throw new ProtocolError("INVALID_PATH", "path is not under the renamed prefix", {
      display,
      fromPrefix,
    });
  }
